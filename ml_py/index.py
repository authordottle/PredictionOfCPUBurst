...
if (input == "macBook"):
    ...
    multicollinearity_drop_columns = [
        "memory_percent", "num_ctx_switches_voluntary"]
    y_column = 'stime'
elif (input == "linux"):
...
for col in df.columns:
    if (df[col] == 0).all(axis=0):
        multicollinearity_drop_columns.append(col)
# print(multicollinearity_drop_columns)

necessary_data_df = df.drop(multicollinearity_drop_columns, axis=1)
# print(df.head())
...
#         df_encode[col].values.reshape(-1, 1)).toarray()
...
# df_encode = df_encode.drop(df_obj_type_cols, axis=1)
# # Verify
# print(df_encode.head())

# 3. Dummy variable trap
# df_encode = pd.get_dummies(data=necessary_data_df, prefix='OHE', prefix_sep='_',
#                            columns=obj_type_cols,
#                            drop_first=True,
#                            dtype='int8')
# # Verify the dummay variable process
# print('Columns in original data frame:', necessary_data_df.columns.values)
# print('Number of rows and columns in the dataset:', necessary_data_df.shape)
...
no_outliers_df = df_encode[(np.abs(stats.zscore(df_encode)) < 3).all(axis=1)]

# Check for missing value
# plt.figure(figsize=(12, 4))
# sns.heatmap(no_outliers_df.isnull(), cbar=False, cmap='viridis', yticklabels=False)
# plt.title('Missing value in the dataset')
# plt.show()

# Finish data preprocessing
preprocessed_df = no_outliers_df
print(preprocessed_df)

# Assign columns variables to X and y
X_df = preprocessed_df.drop(X_drop_columns, axis=1)  # Independent variable

# utime: CPU time spent in user code
# stime: CPU time spent in kernel code
y_df = preprocessed_df[y_column]  # Dependent variable
...
def calculate_vif(data):
    ...
    for i in range(0, x_var_names.shape[0]):
        ...
        # print(r_squared)
        if r_squared == 1:
            print(x_var_names[i] + " r-squared is 1 which causes infinity.")
            exit()
...
ax2.axvline((y_test - y_lr_test_pred).mean(), color='yellow', linestyle='--')
...
y_rfr_train_pred = rfr.predict(X_train_transform)
y_rfr_test_pred = rfr.predict(X_test_transform)
# print(y_train)
...
data = [
    ['Linear regression', lr_train_mse, np.sqrt((lr_train_mse)),
     lr_train_r2, lr_test_mse, np.sqrt((lr_test_mse)), lr_test_r2, lr_mse_mean_scores, lr_rmse_mean_scores, lr_std_scores],
    ['Random forest', rfr_train_mse, np.sqrt((rfr_train_mse)),
     rfr_train_r2, rfr_test_mse, np.sqrt((rfr_test_mse)), rfr_test_r2, rfr_mse_mean_scores, rfr_rmse_mean_scores, rfr_std_scores],
]
results = pd.DataFrame(data,
                       columns=['Method', 'Training MSE', 'Training RMSE',
                                'Training R2', 'Test MSE', 'Test RMSE', 'Test R2', 'Cross-Val MSE Mean', 'Cross-Val RMSE Mean', 'Cross-Val MSE Standard Deviation']
                       )
print(results)
